                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">Daftar Tendik</h3> <a href="/tendik/create" class="btn btn-md btn-success float-sm-right">+ Tambah Akun Tendik</a>
                            </div>
                            <!-- /.card-header -->
                            <div class="card-body">
                                <table id="example1" class="table table-bordered table-striped">
                                    <thead>
                                        <tr>
                                            <th>Nomor</th>
                                            <th>Id [username]</th>
                                            <th>Password <br>[default:54321]</th>
                                            <th>Nama</th>
                                            <th>NIP</th>
                                            <th>Status</th>
                                            <th>Aksi#</th>
                                        ...
                                        @foreach ($data as $index => $item)
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    @if ($item->aktif == 1)
                                                        <form action="/tendik/nonAktifkan" method="post">
                                                            @csrf
                                                    ...
                                                    @else
                                                        <form action="/tendik/aktifkan" method="post">
                                                            @csrf
                                                            <input type="hidden" name="id" value={{$item->id }}>
                                                    ...
                                                    @endif
                                                     @if ($item->aktif == 1)
                                                        @if ($item->password != '54321')
                                                            <form action="/tendik/resetPassword" method="post">
                                                                @csrf
                                                                <input type="hidden" name="id" value={{$item->id }}>
